...
function revealByProgress(progress: number, start: number, end: number) {
...
export function InvitationExperience({ event }: { event: EventData }) {
  ...
  useEffect(() => {
    ...
    const animateToTarget = () => {
      ...
      const tick = () => {
        setProgress((prev) => {
          ...
          if (Math.abs(diff) < 0.0008) return targetRef.current;
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      const isInsideScrollArea =
        !!scrollAreaRef.current && !!target && scrollAreaRef.current.contains(target);

      // Se o progresso já passou do ponto de abertura do scroll (0.68) e estamos dentro da área rolável
      if (progress > 0.68 && isInsideScrollArea) {
        const scrollEl = scrollAreaRef.current;
        // Se o usuário está rolando para cima (deltaY < 0) e o scroll interno está no topo,
        // então podemos começar a fechar o convite.
        if (e.deltaY < 0 && scrollEl.scrollTop === 0) {
          // Permite reduzir o progresso
        } else {
          // Em qualquer outro caso, deixa o scroll interno rolar normalmente.
          return;
        }
      }
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (progress > 0.68 && isInsideScrollArea) {
        const scrollEl = scrollAreaRef.current;
        // dy positivo = movimento para cima (touch move para baixo)
        if (dy > 0 && scrollEl.scrollTop === 0) {
          // Permite reduzir o progresso
        } else {
          // Deixa o scroll interno rolar
          return;
        }
      }

      e.preventDefault();
      targetRef.current = clamp(targetRef.current + dy / 220, 0, 1);
      animateToTarget();
    };
  ...
  useEffect(() => {
    // Quando o progresso chegar a 1 (abertura completa), rola até o formulário
    if (progress === 1 && formSectionRef.current) {
      // Pequeno timeout para garantir que a UI já tenha se atualizado
      const timeoutId = setTimeout(() => {
        formSectionRef.current?.scrollIntoView({
          behavior: "smooth",
          block: "start", // ou "center" se preferir centralizado
        });
      }, 150);
  ...
  const bodyReveal = revealByProgress(progress, 0.30, 0.58);
  const eventReveal = revealByProgress(progress, 0.40, 0.65);
  const formReveal = revealByProgress(progress, 0.52, 0.75);

  const frontRotation = clamp(progress / 0.52, 0, 1) * -180;
  const frontShadow = 0.32 - bodyReveal * 0.2;
  const hintOpacity = 1 - revealByProgress(progress, 0.12, 0.35);
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 409 || data.alreadyConfirmed) {
          ...
          setAlreadyConfirmed(true);
          throw new Error(
            data.error || "Você já enviou uma confirmação para este evento.",
          );
        }
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .marble-panel {
          position: absolute; top: 0; bottom: 0; width: 110px; pointer-events: none;
          ...
          opacity: 0.95; display: none;
        }
        .marble-panel.left  { left: 0; }
        .marble-panel.right { right: 0; }
        @media (min-width: 768px) { .marble-panel { display: block; } }
        ...
        @media (min-width: 600px) { .card-wrap { max-width: 480px; } }
        @media (min-width: 900px) { .card-wrap { max-width: 520px; } }
        ...
        .gold-stripe-bottom {
          ...
          background: linear-gradient(90deg, #5a2e00, #f5d96a 28%, #c08b10 50%, #f0cf60 72%, #5a2e00);
          border-radius: 0 0 28px 28px; z-index: 3;
        }
        ...
        @media (min-width: 480px) { .card-back-inner { padding: 28px 30px 28px; } }

        .heading-launch {
          font-size: clamp(11px, 3vw, 15px);
          font-weight: 300; letter-spacing: 0.22em;
          color: #c8920e; line-height: 1; margin-bottom: 3px;
        }
        .heading-brand {
          font-size: clamp(16px, 4.5vw, 24px);
          font-weight: 300; letter-spacing: 0.08em;
          color: #08245b; line-height: 1.1; margin-bottom: 10px;
        }
        .heading-host {
        ...
        .body-text {
          ...
          line-height: 1.55; color: #1e2a4a; text-align: center;
        }
        .body-text p { margin-bottom: 8px; }
        .body-text p:last-child { margin-bottom: 0; }

        .event-block {
          border: 1px solid rgba(200,154,16,0.35); border-radius: 14px;
          background: rgba(8,36,91,0.07);
          padding: 10px 14px; margin: 14px auto 0; max-width: 320px;
        }
        .event-row {
          display: flex; align-items: center; justify-content: center;
          gap: 7px; font-size: clamp(14px, 3.8vw, 17px);
          color: #08245b; font-weight: 600; margin-bottom: 7px;
        }
        .event-icon { color: #c8920e; }
        .event-location {
          font-size: clamp(11px, 2.8vw, 12.5px); line-height: 1.45;
        ...
        .form-lbl {
          ...
          color: #c8920e; margin-bottom: 9px;
        }
        ...
        .field {
          ...
          color: #0e244f; font-family: 'Montserrat', sans-serif;
          outline: none; transition: border-color .18s, box-shadow .18s;
        }
        .field::placeholder { color: #94a3b8; }
        .field:focus { border-color: #d6b146; box-shadow: 0 0 0 3px rgba(214,177,70,.20); }

        .companion-block {
          border-radius: 10px; border: 1.5px solid rgba(200,154,16,0.28);
          background: rgba(255,255,255,0.90); padding: 7px 12px; color: #0e244f;
        }
        .companion-lbl {
        ...
        .status-box { margin-top: 8px; border-radius: 10px; padding: 8px 12px; font-size: 12px; }
        .status-ok  { background: rgba(16,185,129,.12); color: #065f46; }
        .status-err { background: rgba(239,68,68,.12); color: #7f1d1d; }
        ...
        .cover-convite {
          ...
          color: #e8c963; white-space: nowrap;
          text-shadow: 0 2px 18px rgba(0,0,0,0.35); letter-spacing: 3px;
        }
        .cover-ribbon {
          position: absolute; left: 0; right: 0;
          top: calc(56% - 10px); height: 22px;
        ...
        .cover-bow-wrap {
          ...
          transform: translate(-50%, -50%);
          width: clamp(210px, 64%, 300px); z-index: 5;
        }
        .dot {
        ...
        <div className="card-wrap">
          ...
          <div className="card-back">
            ...
            <div
              ...
              style={{
                opacity: bodyReveal,
                transform: `translateY(${22 - bodyReveal * 22}px)`,
                filter: `blur(${(1 - bodyReveal) * 5}px)`,
                overflowY: progress > 0.68 ? "auto" : "hidden",
            ...
            >
              ...
              <div
                ...
                style={{
                  opacity: eventReveal,
                  transform: `translateY(${14 - eventReveal * 14}px)`,
                }}
              >
              ...
              <div
                ...
                style={{
                  opacity: formReveal,
                  transform: `translateY(${18 - formReveal * 18}px)`,
                  pointerEvents: formReveal > 0.9 ? "auto" : "none",
                }}
              >
                  ...
                  <form className="fields" onSubmit={handleSubmit}>
                    ...
                    {form.hasCompanion === "yes" && (
                      <input
                        ...
                        onChange={(e) =>
                          setForm((s) => ({ ...s, companionName: e.target.value }))
                        }
